...
const getLineData = (name, index) => {
  const count = [100, 200, 300, 400];

  const list = [];
  for (let i = 0; i < 10; i++) {
    const time = dayjs().subtract(i, 'days').format('MM-DD');
    const itemCount = randomNumber(count[index], count[index + 1]);
...
//         },
//         count: () => randomNumber(count[index], count[index + 1]),
//         name: name,
//       },
//     ],
//   });
...
export const getOverviewData = () => {
  ...
  const generateOverviewData = () => {
    const overviewData = [];
    for (let i = 0; i < 4; i++) {
      overviewData.push(randomNumber(0, 10000));
    }
    return overviewData;
  };

  // 生成图表数据
  const generateChartData = (legend) => {
    const list = [];
    legend.forEach((name, index) => {
      list.push(...getLineData(name, index));
    });
    return list;
  };
  const overviewData = generateOverviewData();
  const chartData = generateChartData(legend);
...
//     //       result[name] = randomNumber(0, 100);
//     //     });
...
export const getActivityList = () => {
  const names = ['分享量', '评论量', '点赞量'];
  const list = [];

  for (let i = 0; i < 3; i++) {
    const name = names[i];
    const count = randomNumber(1000, 10000);
